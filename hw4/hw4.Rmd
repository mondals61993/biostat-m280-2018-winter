...
```{r eval = FALSE}
 model_data <- flights_tbl %>%
    ...
    select(year, month, arrdelay, depdelay, distance, uniquecarrier, description, diverted)
```
...
data_2008 <- flights_tbl %>%
  ...
  filter(year == 2008) %>%
  left_join(airlines_tbl, by = c("uniquecarrier" = "code")) %>%
  select(year, month, arrdelay, depdelay, distance, uniquecarrier, description, origin, dest)
```
